...
type PeerToPeerClient interface {
	...
	RelayEvent(ctx context.Context, in *Event, opts ...grpc.CallOption) (*Empty, error)
	RelayEventStream(ctx context.Context, opts ...grpc.CallOption) (PeerToPeer_RelayEventStreamClient, error)
...
func (c *peerToPeerClient) RelayEvent(ctx context.Context, in *Event, opts ...grpc.CallOption) (*Empty, error) {
	out := new(Empty)
	err := c.cc.Invoke(ctx, "/p2p.api.PeerToPeer/RelayEvent", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *peerToPeerClient) RelayEventStream(ctx context.Context, opts ...grpc.CallOption) (PeerToPeer_RelayEventStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &PeerToPeer_ServiceDesc.Streams[1], "/p2p.api.PeerToPeer/RelayEventStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &peerToPeerRelayEventStreamClient{stream}
	return x, nil
}

type PeerToPeer_RelayEventStreamClient interface {
	Send(*Event) error
	CloseAndRecv() (*Empty, error)
	grpc.ClientStream
}

type peerToPeerRelayEventStreamClient struct {
	grpc.ClientStream
}

func (x *peerToPeerRelayEventStreamClient) Send(m *Event) error {
	return x.ClientStream.SendMsg(m)
}

func (x *peerToPeerRelayEventStreamClient) CloseAndRecv() (*Empty, error) {
...
type PeerToPeerServer interface {
	...
	RelayEvent(context.Context, *Event) (*Empty, error)
	RelayEventStream(PeerToPeer_RelayEventStreamServer) error
...
func (UnimplementedPeerToPeerServer) RelayEvent(context.Context, *Event) (*Empty, error) {
	return nil, status.Errorf(codes.Unimplemented, "method RelayEvent not implemented")
}
func (UnimplementedPeerToPeerServer) RelayEventStream(PeerToPeer_RelayEventStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method RelayEventStream not implemented")
}
...
func _PeerToPeer_RelayEvent_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Event)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PeerToPeerServer).RelayEvent(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/p2p.api.PeerToPeer/RelayEvent",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PeerToPeerServer).RelayEvent(ctx, req.(*Event))
	}
	return interceptor(ctx, in, info, handler)
}

func _PeerToPeer_RelayEventStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(PeerToPeerServer).RelayEventStream(&peerToPeerRelayEventStreamServer{stream})
}

type PeerToPeer_RelayEventStreamServer interface {
	SendAndClose(*Empty) error
	Recv() (*Event, error)
	grpc.ServerStream
}

type peerToPeerRelayEventStreamServer struct {
	grpc.ServerStream
}

func (x *peerToPeerRelayEventStreamServer) SendAndClose(m *Empty) error {
	return x.ServerStream.SendMsg(m)
}

func (x *peerToPeerRelayEventStreamServer) Recv() (*Event, error) {
	m := new(Event)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var PeerToPeer_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "RelayEvent",
			Handler:    _PeerToPeer_RelayEvent_Handler,
		},
		{
			MethodName: "Health",
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "RelayEventStream",
			Handler:       _PeerToPeer_RelayEventStream_Handler,
			ClientStreams: true,
		},
	},
